...
namespace EventData {
	...
	struct GameBinding :public Binding {
		...
		void ReadIn(Attributes& stream) override {
			while (!stream.eof()) {
				auto keyattributes = KeyProcessing::ExtractAttributesToStream(stream.GetWord());
	...
	struct GUIBinding : public Binding {
		...
		void ReadIn(Attributes& attributes) override {
			...
			while (!attributes.eof()) {
				auto keyattributes = KeyProcessing::ExtractAttributesToStream(attributes.GetWord());
