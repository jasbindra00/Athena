...
void State_LevelEditor::OnCreate(){
	...
	auto atlas_map_elt = interfaces.at("RIGHT_PANEL")->GetElement<GUILabel>("ATLAS_MAP");
	selector.setPosition(atlas_map_elt->GetGlobalPosition());
	selector.setSize(atlas_map_elt->GetSize());
...
void State_LevelEditor::SetSelectorPosition(const bool& left) {
	sf::Clock c;
	float clockspeed = 0.123;
	while (c.getElapsedTime().asSeconds() < clockspeed) { //limit the selection speed
	...
	interfaces.at("RIGHT_PANEL")->GetElement<GUILabel>("ACTIVE_TILE")->GetActiveStyle().ReadIn(STYLE_ATTRIBUTE::BG_TEXTURE_RECT, sf::IntRect{ std::move(atlas_map_sprite_coord), static_cast<sf::Vector2i>(atlas_tile_size) });
